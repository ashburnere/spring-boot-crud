...
/**
 * 
 * Unit test which only tests the book controller. It uses @WebMvcTest to
 * disable full auto configuration and test only configurations for the MVC will
...
public class BookControllerTest {
	...
	@Test
	public void postingANewBookShouldCreateANewBookInDB() throws JsonProcessingException, Exception {
		BookRequest request = new BookRequest();
		request.setAuthor("Duke");
		request.setIsbn("1337");
	...
	public void allBooksEndpointShouldReturnTwoBooks() throws Exception {
		...
		when(bookService.getAllBooks()).thenReturn(
				Arrays.asList(createBook(1L, "Java 11", "Duke", "1337"), createBook(2L, "Java EE 8", "Duke", "1338")));

		this.mockMvc.perform(get("/api/books")).andExpect(status().isOk())
				.andExpect(content().contentType("application/json")).andExpect(jsonPath("$", hasSize(2)))
				.andExpect(jsonPath("$[0].title", is("Java 11"))).andExpect(jsonPath("$[0].author", is("Duke")))
				.andExpect(jsonPath("$[0].isbn", is("1337"))).andExpect(jsonPath("$[0].id", is(1)));
	}
	...
	public void getBookWithIdOneShouldReturnABook() throws Exception {
		...
		this.mockMvc.perform(get("/api/books/1")).andExpect(status().isOk())
				.andExpect(content().contentType("application/json")).andExpect(jsonPath("$.title", is("Java 11")))
				.andExpect(jsonPath("$.author", is("Duke"))).andExpect(jsonPath("$.isbn", is("1337")))
	...
	public void updateBookWithKnownIdShouldUpdateTheBook() throws Exception {
		...
		this.mockMvc
				.perform(put("/api/books/1").contentType(MediaType.APPLICATION_JSON)
						.content(objectMapper.writeValueAsString(bookRequest)))
				.andExpect(status().isOk()).andExpect(content().contentType("application/json"))
				.andExpect(jsonPath("$.title", is("Java 12"))).andExpect(jsonPath("$.author", is("Duke")))
				.andExpect(jsonPath("$.isbn", is("1337"))).andExpect(jsonPath("$.id", is(1)));
	...
	public void updateBookWithUnknownIdShouldReturn404() throws Exception {
		...
		this.mockMvc.perform(put("/api/books/42").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(bookRequest))).andExpect(status().isNotFound());
	}
